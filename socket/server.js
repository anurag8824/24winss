...
httpServer.listen(8011, (req, res) => {
    console.log("server 8011");
});

mongoose.connect("mongodb+srv://24winsin:kLHNtv6h3zb4bYVU@cluster0.01szpys.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0", {
    
}, (err) => {
    if (!err) {
        console.log(`You are connected with db`);
    } else {
        console.log(`You are not connected with db`);
    }
})
...
wss.on('connection', function (ws) { 
    ...
    case "resultAPI": {
        const game = await Game.find({ $or: [{ Status: "running" }, { Status: "pending" }, { Status: "conflict" }] }).populate("Created_by").populate("Created_by" , "Name  avatar _id" ).populate("Accepetd_By" , "Name  avatar _id" ).populate("Winner" , "Name  avatar _id" );
    ...
    case "game_seen": {
        ...
        const runningBattle = await Game.find({ $or: [{ Status: "running" }, { Status: "pending" }, { Status: "conflict" }] }).populate("Created_by" , "Name  avatar _id" ).populate("Accepetd_By" , "Name  avatar _id" ).populate("Winner" , "Name  avatar _id" );
    ...
    case 'roomCode': {
        ...
        if (localGame.Status != "completed" && localGame.Status != "cancelled" && localGame.Status == "requested" ) {
                ...
                localGame.Winner_closingbalance = Winner_closingbalance; localGame.Loser_closingbalance = Loser_closingbalance;
                localGame.save();
                ...
                await prevCreated(localGame.Created_by, localGame.Accepetd_By);
                    // await axios.get('http://23.22.4.82:3343/api/roomcode')
                    await axios.get('http://23.22.1.82:4545/v1/v2/rooms/new-code-api')
                    .then(res => {
                        if (res.status == 200) {
                ...
                const status = await Game.find({ $or: [{ Status: "new" }, { Status: "requested" }, { Status: "running" }] }).populate("Created_by" , "Name  avatar _id" ).populate("Accepetd_By" , "Name  avatar _id" ).populate("Winner" , "Name  avatar _id" );
...
// mongoose.connect(
//   "mongodb+srv://24winsin:kLHNtv6h3zb4bYVU@cluster0.01szpys.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0",
//   {
...
//             await axios
//               .get('http://23.22.1.82:4545/v1/v2/rooms/new-code-api')
//               .then((res) => {
