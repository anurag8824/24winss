...
router.post('/challenge/runningon/:id', Auth, async (req, res) => {
    try {
        const data = await Game.findById(req.params.id)
        data.Game_Ammount = req.body.bonus
        data.Status = 'running';
        data.save()
...
router.post('/challenge/cancelledon/:id', Auth, async (req, res) => {
    try {
        const data = await Game.findById(req.params.id)
        data.Game_Ammount = req.body.bonus
...
router.post("/challange/create", Auth, async (req, res) => {
        ...
        // Check game amount validity
        if (Game_Ammount < 50 || Game_Ammount > 30000 || Game_Ammount % 50 !== 0) {
            return res.send({ msg: 'Game amount should be in denomination of 50 and between 50 and 30000' });
        }

        // Check user's wallet balance
        if (user.Wallet_balance < Game_Ammount) {
            return res.send({ msg: 'Insufficient balance' });
        }

        // Check game battle limit
        const newGameCount = await Game.countDocuments({ Status: "new", Created_by: req.user.id });
        if (newGameCount >= 2) {
            return res.send({ msg: 'You can set a maximum of 2 battles.' });
        }

        // Check previous games
        const prevGameQuery = {
            $or: [
                { $and: [{ Status: "pending" }, { $or: [{ Created_by: req.user.id }, { Accepetd_By: req.user.id }] }] },
                { $and: [{ Status: "conflict" }, { $or: [{ Created_by: req.user.id, Creator_Status: null }, { Accepetd_By: req.user.id, Acceptor_status: null }] }] },
                { $and: [{ Status: "running" }, { $or: [{ Created_by: req.user.id }, { Accepetd_By: req.user.id }] }] }
            ]
        };

        // Find the previous games
        const prevGame = await Game.find(prevGameQuery);

        // Check if the user has already enrolled
        if (prevGame.length > 0) {
            return res.send({ msg: 'You have already enrolled in a game' });
        }

        // Check if there is already a game with the same amount
        const sameGame = await Game.findOne({ Status: "new", Created_by: req.user.id, Game_Ammount });
        if (sameGame) {
            return res.send({ msg: 'You cannot create a challenge with the same amount.' });
        }
        ...
        // Set timeout to delete the game after 2 minutes if not accepted
        setTimeout(async (ID) => {
...
//     try {
//         const user = await User.findById(req.user.id);
//         if (Game_Ammount >= 50 && Game_Ammount <= 20000) {
...
//         }
//         else {
//             res.send({ msg: 'Game amount should be Greater then 50 and less then 20000' })
//         }
...
router.get("/challange/drop_challange", Auth, async (req, res) => {
    try {
        const status = await Game.find({ $or: [{ check_Status: "active" }] }).populate("Created_by" , "Name Phone avatar _id" ).populate("Accepetd_By" , "Name Phone avatar _id" ).populate("Winner" , "Name Phone avatar _id" ).sort({ updatedAt: -1 })
        res.status(200).send(status)
    } catch (e) {
        res.status(400).send(e)
    }
...
router.get("/challange/all", Auth, async (req, res) => {
    try {
        ...
        ).populate("Created_by" , "Name avatar _id" ).populate("Accepetd_By" , "Name avatar _id" ).populate("Winner" , "Name avatar _id" )
        res.status(200).send(status)
        ...
        if (game.Status != "cancelled" && game.Status != "completed") {
            ...
            if (game.Creator_Status == null || game.Acceptor_status == null) {
                ...
                if (updateResult != null) {
                    ...
                    user1.hold_balance = (user1.hold_balance - game.Game_Ammount);
                    user2.hold_balance = (user2.hold_balance - game.Game_Ammount);
...
async function update_wallet(winID, loseID, gameAmount, winnAmount,deductedWithdrawl) {
    ...
    winner.withdrawAmount += winner.Wallet_balance
    // (winnAmount + gameAmount);

    losser.loseAmount+=gameAmount;
    if(winner.hold_balance > 0){
        winner.hold_balance = (winner.hold_balance - gameAmount);    
    }
    
    if(losser.hold_balance > 0){
        losser.hold_balance = (losser.hold_balance - gameAmount);    
    }
...
router.put("/challange/accept/:id", Auth, async (req, res) => {
    try {
        ...
        if ((game.Status) != "cancelled" && game.Status != "completed" && game.Status != "running" && game.Status == "new") {
            ...
            if(creatorEnrolled && creatorEnrolled.length>0)
            {
                //await game.delete();
                game.Status = "drop"
                await game.save()
        ...
        console.error('gamererror:', e)
            // res.status(400).send('game accept error line 1168')
